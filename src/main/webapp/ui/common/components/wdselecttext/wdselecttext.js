...
var selectBackground = "#FF6600"; // 被选中的背景颜色
...
function selecttextInfo() {
	...
	$('.WDInput').each(function() {
		// code
		if ($("#" + this.id.split('_')[1]).length == 0) {
			var hidtxt = "<input type='hidden' id='" + this.id.split('_')[1] + "' name='" + this.id.split('_')[1] + "' title='"+this.title+"'/>";
	...
	$(".WDInput").bind("keydown", function(event) {
		...
		switch (keyCode) {
			case 40: // 下
				...
				$("#dmBody").find('tr').eq(hightLightIdx).css("background-color", "#FF6600");
...
function mOver(tableRow) {
	$("#dmBody").find('tr').each(function() {
		if (!$(this).hasClass("wd-selectrow")) {
			...
			$(tableRow).css("background-color", "#FF6600");
...
function setSelectRow($WDInput) {
	$("#dmBody").find('tr').each(function() {
		var tmp = $("#" + $WDInput[0].id.split("_")[1]).val().split(',');
...
function selectInputValue(obj, $WDInput) {
	...
	if (multiselect) {
		...
		if (oldCode.indexOf(newCode + ",") == -1) { // 选择不同值时记录选择信息
			var temp = $("#" + $WDInput[0].name.split('_')[1]).val();
			...
			$("#" + $WDInput[0].id.split('_')[1]).val($("#" + $WDInput[0].id.split('_')[1]).val() + $.trim($(obj).children().eq(0).text()) + ",");
		} else {
		...
		$("#" + $WDInput[0].id.split('_')[1]).val($.trim($(obj).children().eq(0).text()));
		$("#dmLayer").hide();
		if (oldCode != newCode) {
			onCodeChange($WDInput);
		}
	...
	if (multiselect) {
		...
		for (var i = 0; i < json_date.data.length; i++) {
			if (json_date.data[i][wdValue] == $WDInput.val()) {
				$("#" + $WDInput[0].name.split('_')[1]).val(json_date.data[i][wdKey]);
				flg = true;
				break;
			}
		}
